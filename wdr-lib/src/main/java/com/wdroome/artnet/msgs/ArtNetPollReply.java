...
public class ArtNetPollReply extends ArtNetMsg
{
	...
	public Inet4Address m_bindIpAddr = ArtNetMsgUtil.getZeroIpAddr();
	...
	public ArtNetPollReply(byte[] buff, int off, int length, Inet4Address fromAddr)
	{
		...
		off += 4;
		m_ipPort = ArtNetMsgUtil.getLittleEndInt16(buff, off);
		off += 2;
		m_firmwareVers = ArtNetMsgUtil.getBigEndInt16(buff, off);
		off += 2;
		m_netAddr = buff[off++] & 0xff;
		m_subNetAddr = buff[off++] & 0xff;
		m_oem = ArtNetMsgUtil.getBigEndInt16(buff, off);
		off += 2;
		m_ubea = buff[off++] & 0xff;
		m_status1 = buff[off++] & 0xff;
		m_esta = ArtNetMsgUtil.getLittleEndInt16(buff, off);
		off += 2;
		...
		off += 64;
		m_numPorts = ArtNetMsgUtil.getBigEndInt16(buff, off);
		off += 2;
		ArtNetMsgUtil.copyBytes(m_portTypes, 0, buff, off, 4);
		off += 4;
		ArtNetMsgUtil.copyBytes(m_goodInput, 0, buff, off, 4);
		off += 4;
		ArtNetMsgUtil.copyBytes(m_goodOutput, 0, buff, off, 4);
		off += 4;
		...
		ArtNetMsgUtil.copyBytes(m_macAddr, 0, buff, off, 6);
		off += 6;
		if (length >= size()) {
			m_bindIpAddr = ArtNetMsgUtil.getIpAddr(buff, off);
			off += 4;
			m_bindIndex = buff[off++] & 0xff;		
			m_status2 = buff[off++] & 0xff;
			ArtNetMsgUtil.copyBytes(m_goodOutputB, 0, buff, off, 4);
			off += 4;
	...
	public int putData(byte[] buff, int off)
	{
		...
		off += 4;
		ArtNetMsgUtil.putLittleEndInt16(buff, off, m_ipPort);
		off += 2;
		ArtNetMsgUtil.putBigEndInt16(buff, off, m_firmwareVers);
		off += 2;
		buff[off++] = (byte)m_netAddr;
		buff[off++] = (byte)m_subNetAddr;
		ArtNetMsgUtil.putBigEndInt16(buff, off, m_oem);
		off += 2;
		buff[off++] = (byte)m_ubea;
		buff[off++] = (byte)m_status1;
		ArtNetMsgUtil.putLittleEndInt16(buff, off, m_esta);
		off += 2;
		ArtNetMsgUtil.putString(buff, off, 18, m_shortName);
		off += 18;
		ArtNetMsgUtil.putString(buff, off, 64, m_longName);
		off += 64;
		ArtNetMsgUtil.putString(buff, off, 64, m_nodeReport);
		off += 64;
		ArtNetMsgUtil.putBigEndInt16(buff, off, m_numPorts);
		off += 2;
		ArtNetMsgUtil.copyBytes(buff, off, m_portTypes, 0, 4);
		off += 4;
		ArtNetMsgUtil.copyBytes(buff, off, m_goodInput, 0, 4);
		off += 4;
		ArtNetMsgUtil.copyBytes(buff, off, m_goodOutput, 0, 4);
		off += 4;
		...
		ArtNetMsgUtil.zeroBytes(buff, off, 3);	// spare
		off += 3;
		buff[off++] = (byte)m_style;
		ArtNetMsgUtil.copyBytes(buff, off, m_macAddr, 0, 6);
		off += 6;
		ArtNetMsgUtil.putIpAddr(buff, off, m_bindIpAddr);
		off += 4;
		buff[off++] = (byte)m_bindIndex;		
		buff[off++] = (byte)m_status2;
		ArtNetMsgUtil.zeroBytes(buff, off, 26);	// filler
		off += 26;
		return off;
	}
	...
	public String toString()
	{
		...
		ArtNetMsgUtil.append(b, "firmwareVers", m_firmwareVers);
		ArtNetMsgUtil.appendHex(b, "netAddr", m_netAddr);
		ArtNetMsgUtil.appendHex(b, "subNetAddr", m_subNetAddr);
		ArtNetMsgUtil.append(b, "oem", m_oem);
		ArtNetMsgUtil.appendHex(b, "status1", m_status1);
		ArtNetMsgUtil.append(b, "esta", m_esta);
		ArtNetMsgUtil.append(b, "shortName", m_shortName);
		ArtNetMsgUtil.append(b, "longName", m_longName);
		ArtNetMsgUtil.append(b, "nodeReport", m_nodeReport);
		ArtNetMsgUtil.append(b, "numPorts", m_numPorts);
		ArtNetMsgUtil.append(b, "portTypes", m_portTypes);
		ArtNetMsgUtil.append(b, "goodInput", m_goodInput);
		ArtNetMsgUtil.append(b, "goodOutput", m_goodOutput);
		ArtNetMsgUtil.append(b, "swIn", m_swIn);
		ArtNetMsgUtil.append(b, "swOut", m_swOut);
		ArtNetMsgUtil.appendHex(b, "acnPrty", m_acnPriority);
		ArtNetMsgUtil.appendHex(b, "swMacro", m_swMacro);
		ArtNetMsgUtil.appendHex(b, "swRemote", m_swRemote);
		ArtNetMsgUtil.appendHex(b, "style", m_style);
		ArtNetMsgUtil.append(b, "macAddr", m_macAddr);
		ArtNetMsgUtil.append(b, "bindIpAddr", m_bindIpAddr);
		ArtNetMsgUtil.append(b, "bindIndex", m_bindIndex);
		ArtNetMsgUtil.appendHex(b, "status2", m_status2);
		ArtNetMsgUtil.append(b, "goodOutputB", m_goodOutputB);
		ArtNetMsgUtil.appendHex(b, "status3", m_status3);
		ArtNetMsgUtil.append(b, "defUID", m_defRespUID.toString());
		ArtNetMsgUtil.append(b, "fromAddr", getFromAddr());
		b.append('}');
		return b.toString();
	}
