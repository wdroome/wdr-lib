 ...
 * @author wdr
 */
public class PropertyMap_CIDRv4 extends PropertyMap<CIDRAddress>
{
	/**
	...
	static {
		...
		try {
			cidr = new CIDRAddress("0.0.0.0/0", "ipv4");
		} catch (Exception e) {
		}
		ZERO_LEN_CIDR = cidr;
	}
	
	public PropertyMap_CIDRv4()
	{
		super(EntityMakers.ENT_TYPE_CIDRV4, EntityMakers.g_makeCIDR, false);
	}
	...
	{
		if (!cidr.isIPV4()) {
			throw new IllegalArgumentException("CIDR4PropertyMap.getProp(): CIDR is not v4");
		}
	...
	{
		if (!cidr.isIPV4()) {
			throw new IllegalArgumentException("CIDR4PropertyMap.setProp(): CIDR is not v4");
		}
